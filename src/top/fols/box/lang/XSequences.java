...
public class XSequences {
	...
	public static <A,E> void arraycopy(XInterfacesSequence originArray, int originarrayCopyOffIndex, XInterfacesSequence newArray, int off, int len) {
		for (int i = 0;i < len;i++) newArray.set(i + off, originArray.get(i + originarrayCopyOffIndex));
	}
	...
	public static boolean deepEqualsRange(XInterfacesSequence originalArray, int originalArrayOff, XInterfacesSequence objectArray, int objectArrayOff, int len) {
		...
		for (int i = 0;i < len; i++) {
			if (originalArray.isArray(i + originalArrayOff)) {
				...
				if (!deepEquals(o, o1)) return false;
			} else {
	...
	public static int deepIndexOf(XInterfacesSequence originalArray, Object value, int index) {
		...
		for (int i = index; i < length; i++) {
			if (originalArray.isArray(i)) {
				if (valueIsArray) {
					if (deepEquals(originalArray.get(i), value)) {
						return i;
					}
				}
