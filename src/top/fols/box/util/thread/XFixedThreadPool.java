...
public class XFixedThreadPool {
	/*
	 运行区 等待区....
	 用户post到等待区。。。
	 根据maxRunningCount 判断等待区任务是否可以添加到运行区
	 */
	...
	private static class BooleanObj {
		private boolean b;
		public BooleanObj() {}
		public BooleanObj(boolean b) {this.b = b;}
	...
	public static abstract class Run implements XInterfaceInterruptable{
	...
	private static class ThreadMessage {
		public static enum stateType {running,waiting;}
	...
	public XFixedThreadPool stopAndWaitComplete(List<Run> tasks) {
		synchronized (sync) {
			...
			for (Run r:tasks) {
				if (null != r) {
					while (!r.runComplete) {
						try {Thread.sleep(1);} catch (InterruptedException e) {e = null;}
						continue;
					}
				}
			}
