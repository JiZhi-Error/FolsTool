...
public class XEncodingDetect {
	public static final enum EncodingType {
		...
		static public String toJavaEncodingName(EncodingType type)
		{
			switch(type)
			{
				...
				case HZ:return  "ASCII"; // What to put here? Sun doesn't support HZ
				case BIG5:return  "BIG5";
				case CNS11643:return  "EUC-TW";
				case UTF8:return  "UTF-8";
				case UNICODE:return  "Unicode";
				case EUC_KR:return  "EUC_KR";
				case CP949:return  "MS949";

				case ISO2022CN:return  "ISO2022CN";
				case ISO2022JP:return  "ISO2022JP";
				case ISO2022KR:return  "ISO2022KR";

				case SJIS:return  "SJIS";
				case EUC_JP:return  "EUC_JP";
				case ASCII:return  "ASCII";

				case OTHER:return  "ISO8859_1";

				case DEAULT:return XStaticSystem.getDefaultCharsetName();
			}
	...
	public static String getJavaEncode(File bytes) throws IOException {
		return EncodingType.toJavaEncodingName(new BytesEncodingDetect().detectEncoding(bytes));
	}
	public static Charset getJavaEncode2Charset(File bytes) throws IOException {
	...
	static class BytesEncodingDetect {
		 ...
		 */
 		private EncodingType detectEncoding(XInterfaceSequenceBigByteIO rawtext) throws IOException {
			...
			switch (encoding_guess) {
				case GB2312 : return EncodingType.GB2312;
				case GBK : return EncodingType.GBK;
				case GB18030 : return EncodingType.GB18030;
				case HZ : return EncodingType.HZ;
				case BIG5 : return EncodingType.BIG5;
				case CNS11643 : return EncodingType.CNS11643;
				case UTF8 : return EncodingType.UTF8;
				case UNICODE : return EncodingType.UNICODE;
				case ISO2022CN : return EncodingType.ISO2022CN;
				case EUC_KR : return EncodingType.EUC_KR;
				case CP949 : return EncodingType.CP949;
				case ISO2022KR : return EncodingType.ISO2022KR;
				case SJIS : return EncodingType.SJIS;
				case EUC_JP : return EncodingType.EUC_JP;
				case ISO2022JP : return EncodingType.ISO2022JP;
				case ASCII : return EncodingType.ASCII;
				case OTHER : return EncodingType.OTHER;
			}
			return null;
		}
		...
		private byte[] seekNeedByte(File f, int length, long maxForLength) throws IOException {
			...
			byte[] bytes = seekNeedByte(input, length, maxForLength);
			input.close();
			return bytes;
		}
		private byte[] seekNeedByte(InputStream input, int length, long maxForLength) throws IOException {
			long filelength = input.available();
			if (filelength < 1)
		...
		private byte[] seekNeedByte(byte[] f, int length, int maxForLength) {
...
// asciichars++;
					...
					dbchars++;
					if ((byte) 0xA1 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= (byte) 0xF7 && (byte) 0xA1 <= rawtext.byteAt(i + 1)
...
// System.out.println("original row " + row + " column " + column);
					...
					} else if ((byte) 0x81 <= rawtext.byteAt(i)
							   && rawtext.byteAt(i) <= (byte) 0xFE
							   && // Extended GB range
							   (((byte) 0x80 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0xFE) || ((byte) 0x40 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0x7E))) {
						gbchars++;
						totalfreq += 500;
...
// System.out.println("extended row " + row + " column " + column + " rawtext.get(i) " + rawtext.get(i));
...
// asciichars++;
					...
					dbchars++;
					if ((byte) 0xA1 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= (byte) 0xF7 && // Original GB range
						i + 1 < rawtextlen && (byte) 0xA1 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0xFE) {
						gbchars++;
						totalfreq += 500;
						row = rawtext.byteAt(i) + 256 - 0xA1;
						column = rawtext.byteAt(i + 1) + 256 - 0xA1;
// System.out.println("original row " + row + " column " + column);
					...
					} else if ((byte) 0x81 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= (byte) 0xFE
							   && // Extended GB range
							   i + 1 < rawtextlen
							   && (((byte) 0x80 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0xFE) || ((byte) 0x40 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0x7E))) {
						gbchars++;
...
// System.out.println("extended row " + row + " column " + column + " rawtext.get(i) " + rawtext.get(i));
						if (GBK_[row][column] != 0) {
							gbfreq += GBK_[row][column];
							   ...
							   && rawtext.byteAt(i) <= (byte) 0xFE
							   && // Extended GB range
							   i + 3 < rawtextlen && (byte) 0x30 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0x39
							   && (byte) 0x81 <= rawtext.byteAt(i + 2) && rawtext.byteAt(i + 2) <= (byte) 0xFE && (byte) 0x30 <= rawtext.byteAt(i + 3)
							   && rawtext.byteAt(i + 3) <= (byte) 0x39) {
						gbchars++;
						/*
						 * totalfreq += 500; row = rawtext.get(i) + 256 - 0x81; if (0x40 <= rawtext.get(i+1) && rawtext.get(i+1) <= 0x7E) { column =
						 * rawtext.get(i+1) - 0x40; } else { column = rawtext.get(i+1) + 256 - 0x40; } //System.out.println("extended row " + row + "
						 * column " + column + " rawtext.get(i) " + rawtext.get(i)); if (GBKFreq[row][column] != 0) { gbfreq += GBKFreq[row][column]; }
						 */
					}
					i++;
				}
					...
					if (rawtext.byteAt(i + 1) == '{') {
						...
						while (i < rawtextlen - 1) {
							if (rawtext.byteAt(i) == 0x0A || rawtext.byteAt(i) == 0x0D) {
								...
								i++;
								break;
							} else if ((0x21 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= 0x77) && (0x21 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= 0x77)) {
								hzchars += 2;
							...
							} else if ((0xA1 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= 0xF7) && (0xA1 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= 0xF7)) {
								hzchars += 2;
								row = rawtext.byteAt(i) + 256 - 0xA1;
								column = rawtext.byteAt(i + 1) + 256 - 0xA1;
								totalfreq += 500;
...
// asciichars++;
					...
					dbchars++;
					if ((byte) 0xA1 <= rawtext.byteAt(i)
						&& rawtext.byteAt(i) <= (byte) 0xF9
						&& (((byte) 0x40 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0x7E) || ((byte) 0xA1 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0xFE))) {
						bfchars++;
						totalfreq += 500;
						row = rawtext.byteAt(i) + 256 - 0xA1;
						if (0x40 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= 0x7E) {
...
// asciichars++;
					...
					if (0xA1 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= 0xF9 && // Original Big5 range
						((0x40 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= 0x7E) || (0xA1 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= 0xFE))) {
						bfchars++;
						totalfreq += 500;
...
// System.out.println("original row " + row + " column " + column);
						if (Big5_[row][column] != 0) {
							...
							bffreq += 200;
							   ...
							   ((0x40 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= 0x7E) || (0x80 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= 0xFE))) {
						bfchars++;
						totalfreq += 500;
...
// asciichars++;
					...
					dbchars++;
					if (i + 3 < rawtextlen && (byte) 0x8E == rawtext.byteAt(i) && (byte) 0xA1 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0xB0
						&& (byte) 0xA1 <= rawtext.byteAt(i + 2) && rawtext.byteAt(i + 2) <= (byte) 0xFE && (byte) 0xA1 <= rawtext.byteAt(i + 3)
...
// and have expected frequency of use
			...
			for (i = 0; i < rawtextlen - 1; i++) {
				if (rawtext.byteAt(i) == (byte) 0x1B && i + 3 < rawtextlen) { // Escape char ESC
					if (rawtext.byteAt(i + 1) == (byte) 0x24 && rawtext.byteAt(i + 2) == 0x29 && rawtext.byteAt(i + 3) == (byte) 0x41) { // GB Escape $ ) A
						i += 4;
						while (rawtext.byteAt(i) != (byte) 0x1B) {
							dbchars++;
							if ((0x21 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= 0x77) && (0x21 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= 0x77)) {
								isochars++;
								row = rawtext.byteAt(i) - 0x21;
								column = rawtext.byteAt(i + 1) - 0x21;
								totalfreq += 500;
					...
					} else if (i + 3 < rawtextlen && rawtext.byteAt(i + 1) == (byte) 0x24 && rawtext.byteAt(i + 2) == (byte) 0x29
							   && rawtext.byteAt(i + 3) == (byte) 0x47) {
// CNS Escape $ ) G
						i += 4;
						while (rawtext.byteAt(i) != (byte) 0x1B) {
							dbchars++;
							if ((byte) 0x21 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= (byte) 0x7E && (byte) 0x21 <= rawtext.byteAt(i + 1)
					...
					if (rawtext.byteAt(i) == (byte) 0x1B && i + 2 < rawtextlen && rawtext.byteAt(i + 1) == (byte) 0x28 && rawtext.byteAt(i + 2) == (byte) 0x42) { // ASCII:
// ESC
// ( B
						i += 2;
					}
...
// Ignore ASCII, can throw off count
					...
					goodbytes += 2;
					i++;
				} else if (-32 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= -17
						   && // Three bytes
						   i + 2 < rawtextlen && -128 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= -65 && -128 <= rawtext.byteAt(i + 2)
						   && rawtext.byteAt(i + 2) <= -65) {
					goodbytes += 3;
					i += 2;
				}
			}
...
// Allows for some (few) bad formed sequences
		...
		int utf16_probability(XInterfaceSequenceBigByteIO rawtext) throws IOException {
			 ...
			 * if ((rawtext.get(i) & (byte)0x7F) == rawtext.get(i)) { // One byte goodbytes += 1; asciibytes++; } else if ((rawtext.get(i) &
			 * (byte)0xDF) == rawtext.get(i)) { // Two bytes if (i+1 < rawtextlen && (rawtext.get(i+1) & (byte)0xBF) == rawtext.get(i+1)) {
			 * goodbytes += 2; i++; } } else if ((rawtext.get(i) & (byte)0xEF) == rawtext.get(i)) { // Three bytes if (i+2 < rawtextlen &&
			 * (rawtext.get(i+1) & (byte)0xBF) == rawtext.get(i+1) && (rawtext.get(i+2) & (byte)0xBF) == rawtext.get(i+2)) { goodbytes += 3; i+=2; } } }
			 * 
...
// asciichars++;
					...
					dbchars++;
					if ((byte) 0xA1 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= (byte) 0xFE && (byte) 0xA1 <= rawtext.byteAt(i + 1)
						&& rawtext.byteAt(i + 1) <= (byte) 0xFE) {
						krchars++;
						totalfreq += 500;
						row = rawtext.byteAt(i) + 256 - 0xA1;
...
// asciichars++;
					...
					dbchars++;
					if ((byte) 0x81 <= rawtext.byteAt(i)
						&& rawtext.byteAt(i) <= (byte) 0xFE
						&& ((byte) 0x41 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0x5A || (byte) 0x61 <= rawtext.byteAt(i + 1)
						&& rawtext.byteAt(i + 1) <= (byte) 0x7A || (byte) 0x81 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0xFE)) {
						krchars++;
						totalfreq += 500;
						if ((byte) 0xA1 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= (byte) 0xFE && (byte) 0xA1 <= rawtext.byteAt(i + 1)
							&& rawtext.byteAt(i + 1) <= (byte) 0xFE) {
...
// asciichars++;
					...
					if (i + 1 < rawtext.length()
						&& (((byte) 0x81 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= (byte) 0x9F) || ((byte) 0xE0 <= rawtext.byteAt(i) && rawtext.byteAt(i) <= (byte) 0xEF))
						&& (((byte) 0x40 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0x7E) || ((byte) 0x80 <= rawtext.byteAt(i + 1) && rawtext.byteAt(i + 1) <= (byte) 0xFC))) {
						jpchars++;
						totalfreq += 500;
