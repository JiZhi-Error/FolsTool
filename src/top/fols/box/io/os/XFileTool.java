...
public class XFileTool {
	...
	public static byte[] readFile(File file) {
		...
		long length;
		try { 
			randomFile = new RandomAccessFile(file, XStaticFixedValue.FileValue.getRandomAccessFile_Mode_R_String());
	...
	public static boolean saveFile(File file, byte[] bytes) throws IOException {
		...
		RandomAccessFile randomFile = null;
		try { 
			randomFile = new RandomAccessFile(file, XStaticFixedValue.FileValue.getRandomAccessFile_Mode_RWS_String());
	...
	private static void writeNullDataFile0(XRandomAccessFileOutputStream out, long start, long length) throws IOException {
		...
		out.seekIndex(start);
		int bufs = XStream.default_streamByteArrBuffSize;
		if (length <= bufs) {
	...
	public static List<String> getFileList(String filePath, boolean recursion, boolean adddir) throws IOException {
		List<String> List = new XArrayList<>();
	...
	public static List<String> listFilesSort(File filePath, boolean adddir) {
		List<String> d = new XArrayList<>();
		List<String> f = new XArrayList<String>();
		File[] files = filePath.listFiles();
		if (files != null)
			for (File file : files) {
				if (file == null)
