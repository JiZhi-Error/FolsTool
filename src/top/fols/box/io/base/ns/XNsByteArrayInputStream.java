...
public class XNsByteArrayInputStream extends InputStream implements  XInterfacePrivateBuffOperat<byte[]>,XInterfaceStreamLineReader<byte[]>,XInterfacePrivateByteArrayBuffSearchOperat,XInterfacePrivateFixedStreamSizeOperat,XInterfacePrivateFixedStreamIndexOperat {
	...
	private int count;
	// 构造函数：创建一个内容为buf的字节流
	public XNsByteArrayInputStream(byte buf[]) {
		// 初始化“字节流对应的字节数组为buf”
		this.buf = buf;
		// 初始化“下一个要被读取的字节索引号为0”
		this.pos = 0;
	...
	public XNsByteArrayInputStream(byte buf[], int offset, int length) {
		// 初始化“字节流对应的字节数组为buf”
		this.buf = buf;
	...
	public byte[] readLine(byte[] split, boolean resultAddSplitChar) {
		...
		if (pos >= count)
			return null;
		int index = XArrays2.indexOf(buf, split, pos, count);
		if (index == -1) {
